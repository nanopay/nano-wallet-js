...
import { AccountBalanceResponse, AccountInfoResponse, ReceivableWithThreshold } from "./RpControllerc.types";
...
export default class NanoRPC {
    ...
    async postRPC<TRPCResponse = unknown>(data: any, urls = this.rpcUrls, retry = 0): Promise<TRPCResponse> {
        ...
        try {
            ...
            if (!response.ok) {
                console.error(`RPC Error - Bad Status: ${response.status} ${response.statusText}`)
            ...
            const isRetryableError =
                error instanceof SyntaxError && error.message.startsWith("SyntaxError") ||
